...
myapp.controller('RoleUpdateController', function($rootScope, $scope, $state, Session, USER_ROLES, $localStorage,
				$sessionStorage, $ngConfirm, $routeParams, $log, $uibModalInstance, ngTableParams, RoleUpdateService) {
	
	$scope.roleAuthorityUpdateResquest = {};
	
	$scope.initForm = function() {
		var roleUpdateRequest = {
    		id : $localStorage.prevPageData.selectedRole.id
    	};
		RoleUpdateService.init(roleUpdateRequest).then(function success(response) {
    ...
    $scope.updateRole = function () {
		...
		$scope.roleUpdateRequest = {
			id: $scope.role.id,		
			roleName : $scope.role.roleName,
			description : $scope.role.description,
			items : checkedNodes
		}
        RoleUpdateService.update($scope.roleUpdateRequest)
            .then(function success(response) {
            	$uibModalInstance.close();
            	$rootScope.$broadcast("reload-role", { title: "Update Role", message: response.data, notificationType: "success" });
